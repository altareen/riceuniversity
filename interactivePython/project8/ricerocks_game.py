...
| Author:   Alwin Tareen
| Created:  Jul 02, 2015
| Execute:  python3 ricerocks_game.py
|
| This program implements the rice rocks game.
|
| Codeskulptor link:
| https://py2.codeskulptor.org/#user48_MPQTkk1KW3_36.py
|
"""
...
class Sprite:
    ...
    def update(self):
        ...
        # check if the Sprite has lived out its usefulness
...
def group_collide(group, other_object):
    numCollisions = 0
    for pebble in group:
        if pebble.collide(other_object):
            group.remove(pebble)
            numCollisions += 1
...
def group_group_collide(first_group, second_group):
    elemCollide = 0
    for particle in first_group:
        elemCollide = group_collide(second_group, particle)
        if elemCollide > 0:
            first_group.remove(particle)
    return elemCollide
...
def process_sprite_group(sprite_set, canvas):
    for piece in sprite_set:
        piece.draw(canvas)
        if not piece.update():
            sprite_set.remove(piece)
